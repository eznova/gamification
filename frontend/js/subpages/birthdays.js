...
export async function loadBdaysContent(userId, backendUrl, signal) {
    const content = document.getElementById('content'); // Контейнер для вставки шаблона
    const response = await fetch('subpages/birthdays.html');
    ...
    return await fetch(`${backendUrl}/users/get/bdays/${new Date().getMonth() + 1}`, { signal })
        .then(res => res.json())
        .then(bdays => {
